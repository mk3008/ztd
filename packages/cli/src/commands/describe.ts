...
export const COMMANDS = [
  ...
  { name: 'query optional add', summary: 'Add an SSSQL optional search condition branch and refresh query metadata.', useCase: 'Use SSSQL notation, for example (:email is null or users.email = :email), while keeping runtime behavior metadata-backed.' },
  { name: 'query optional refresh', summary: 'Refresh SSSQL optional-condition metadata after SQL edits.', useCase: 'Regenerate metadata when the SQL still owns the intended optional filter shape. See docs/guide/sssql.md for the notation.' },
  { name: 'query optional remove', summary: 'Remove an SSSQL optional search condition branch and refresh query metadata.', useCase: 'Delete optional-condition scaffolding without hand-editing metadata.' },
